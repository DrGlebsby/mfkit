...
License:
--------
Gleb Averochkin, Ivan Zlobin, Ivan Bespalov, Eugeny Alexandrov, Machine learning with physically grounded, interpretable descriptors for polymer property prediction and monomer design, Chemical Engineering Science, Volume 328, 2026, 123732
...
def featurize_to_dict(smiles: str) -> dict:
    """
    Compute all descriptors with their full human-readable names.
    ...
    -------
    dict
        Dictionary {full_feature_name: value}.
    """
    return rename_features(all_chosen_features_eng, featurize(smiles))

def featurize_many(smiles_list) -> pd.DataFrame:
    """
    Featurize multiple SMILES strings and return a pandas DataFrame.

    Invalid SMILES strings or molecules that cannot be featurized are
    represented by rows filled with NaN values.
    """
    rows = []
    ...
    for smiles in smiles_list:
        try:
            ...
            row = {feature: np.nan for feature in all_chosen_features}

        row = {"SMILES": smiles, **row}
        rows.append(row)

    return pd.DataFrame(rows, columns=["SMILES"] + all_chosen_features)